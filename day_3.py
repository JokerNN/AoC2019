...
wire1, wire2 = s_input.split('\n')

wire1 = wire1.split(',')
wire2 = wire2.split(',')

def draw(commands, wire_number):
  cur_x = 0
  cur_y = 0
...
for row_num, row in grid.items():
  for col_num, value in row.items():
    if value == 3:
      print(row_num, col_num)
      crossing_points[(row_num, col_num)] = []
      dist = abs(row_num) + abs(col_num)
...
def draw2(commands, wire_number):
  cur_x = 0
  cur_y = 0
  dist = 0
  for command in commands:
    ...
    for _ in range(length):
      if grid[cur_x][cur_y] == 3:
        print(cur_x, cur_y)
        if len(crossing_points[(cur_x, cur_y)]) < wire_number:
          crossing_points[(cur_x, cur_y)].append(dist)
      if direction == 'L':
        cur_x -= 1
      elif direction == 'R':
        cur_x += 1
...
draw2(wire1, 1)
draw2(wire2, 2)
del crossing_points[(0, 0)]
min_dist = min(crossing_points.values(), key=lambda p: p[0] + p[1])
print(min_dist)
print('Answer 2', min_dist[0] + min_dist[1])